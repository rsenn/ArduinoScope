...
bool
SensorDataHandler::addSensorData(const QString label, const QString unit) {
  if(label.size() == 0)
    return false;
...
bool
SensorDataHandler::removeSensorData(const QString label) {
  data_mutex_.lock();
...
bool
SensorDataHandler::removeSensorData(const int index) {
  data_mutex_.lock();
...
bool
SensorDataHandler::shrinkSensorDataVector(const int size) {
...
void
SensorDataHandler::clearTimeAndSensorDataVectors() {
  data_mutex_.lock();
...
void
SensorDataHandler::clearTimeVectorAndSensorDataValues() {
  data_mutex_.lock();
  // Clear time vector
  data_.first.clear();
  // Clear sensor data values
  for(SensorData data : data_.second) data.values.clear();
  data_mutex_.unlock();
}

bool
SensorDataHandler::renameSensorData(const QString old_label, const QString new_label) {
  data_mutex_.lock();
...
bool
SensorDataHandler::renameSensorData(const int index, const QString new_label) {
  data_mutex_.lock();
...
bool
SensorDataHandler::renameSensorDataUnit(const QString label, const QString new_unit) {
  data_mutex_.lock();
...
bool
SensorDataHandler::renameSensorDataUnit(const int index, const QString new_unit) {
  data_mutex_.lock();
...
int
SensorDataHandler::getSensorDataSize() {
  return (data_.second.size());
}

int
SensorDataHandler::getTimeVectorSize() {
  return (data_.first.size());
}

const SensorDataHandler::TimeSensorDataVectors&
SensorDataHandler::getTimeSensorDataVectors() {
  return data_;
}

const SensorDataHandler::TimeVector&
SensorDataHandler::getTimeVector() {
  return data_.first;
}

const SensorDataHandler::SensorDataVector&
SensorDataHandler::getSensorDataVector() {
  return data_.second;
}

SensorDataHandler::SensorData const*
SensorDataHandler::getSensorData(const QString label) {
  data_mutex_.lock();
...
QStringList
SensorDataHandler::getSensorDataLabels() {
  QStringList tmp;
  for(SensorData data : data_.second) tmp.append(data.label);

  return tmp;
}

QStringList
SensorDataHandler::getSensorDataUnits() {
  QStringList tmp;
  for(SensorData data : data_.second) tmp.append(data.unit);

  return tmp;
}

bool
SensorDataHandler::readCSVFile(const QString file_name) {
...
bool
SensorDataHandler::writeCSVFile(const QString file_name) {
  data_mutex_.lock();
...
bool
SensorDataHandler::pushBackTimedSensorDataValues(const QDateTime time, const QVector<double> sensor_data_values) {
  if(time.isNull())
    return false;
  if(!time.isValid())
    return false;
  ...
  unsigned i(0);
  for(SensorData& data : data_.second) data.values.push_back(sensor_data_values.at(i++));
